// ======================================================
// Shelly Modbus RTU Module + Project Code
...
// ======================================================
// PROJECT CODE: ENERGY METER ID1 + SENSOR ID2
// ======================================================
//
// Energy Meter:
// Slave ID: 1
...
// Register 0x0000 -> Temperature -> number:206
// Register 0x0001 -> Humidity    -> number:207
//
// Baudrate: 9600
// Mode: 8N1
// ======================================================
...
// Energy Meter Virtual Components
...
const READ_LIST = [
  ...
  {
    name: "Energy Meter Active Power",
    SlaveID: 1,
    Function: 4,
    Register: 0x0008,
    Quantity: 1,
  ...
  {
    name: "Energy Meter Total Active Energy",
    SlaveID: 1,
    Function: 4,
    Register: 0x001D,
    Quantity: 2,
...
function pollNextItem() {
  if (readIndex >= READ_LIST.length) {
    readIndex = 0;
    cycleRunning = false;

    print("========== MODBUS CYCLE DONE ==========");

    Timer.set(CYCLE_PAUSE_MS, false, function () {
      startModbusCycle();
    });
  ...
  readmodbus({
    ...
    onValue: function (value, result) {
      setValue(item.VC, value);

      print(item.name + " = " + value);
      print(item.name + " raw = " + result.raw);
    },

    onError: function (err) {
      print("Read error: " + item.name + " / " + JSON.stringify(err));
    },

    onDone: function () {
      readIndex++;

      Timer.set(REQUEST_GAP_MS, false, function () {
        pollNextItem();
      });
    }
  });
...
function startModbusCycle() {
  if (cycleRunning) {
    print("Previous Modbus cycle still running, skip.");
    return;
  }
